...
/// A test case class for testing the `PATCH` HTTP method using the `AsyncNetworkManager`.
///
/// This test case class is a subclass of `AsyncNetworkManagerTestCase`, which provides the testing environment and instance of the `AsyncNetworkManager` with the `.testing` environment configuration. The `PatchHTTPMethodTests` class focuses on testing the `PATCH` HTTP method for specific endpoints using the `AsyncNetworkManager`.
///
/// **Testing the PATCH HTTP Method:**
///
/// The `patch` HTTP method is used to update data on the server by modifying some resources. In this test case class, we define test methods to ensure that the `AsyncNetworkManager` correctly handles `PATCH` requests, performs the network communication, and decodes the server response into the expected model objects.
///
/// **Usage:**
///
/// To use this test case class, run the individual test methods or all test methods collectively to verify the correctness of the `PATCH` HTTP method implementation in the `AsyncNetworkManager`. The test methods are automatically executed by the testing framework, and any failed assertions will be reported as test failures.
///
/// **Example:**
///
/// ```swift
