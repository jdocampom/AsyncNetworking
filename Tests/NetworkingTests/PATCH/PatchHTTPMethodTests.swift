...
/// A test case class for testing the `PATCH` HTTP method using the `NetworkManager`.
///
/// This test case class is a subclass of `AsyncNetworkManagerTestCase`, which provides the testing environment and instance of the `NetworkManager` with the `.testing` environment configuration. The `PatchHTTPMethodTests` class focuses on testing the `PATCH` HTTP method for specific endpoints using the `NetworkManager`.
///
/// **Testing the PATCH HTTP Method:**
///
/// The `patch` HTTP method is used to update data on the server by modifying some resources. In this test case class, we define test methods to ensure that the `NetworkManager` correctly handles `PATCH` requests, performs the network communication, and decodes the server response into the expected model objects.
///
/// **Usage:**
///
/// To use this test case class, run the individual test methods or all test methods collectively to verify the correctness of the `PATCH` HTTP method implementation in the `NetworkManager`. The test methods are automatically executed by the testing framework, and any failed assertions will be reported as test failures.
///
/// **Example:**
///
/// ```swift
/// class MyPatchHTTPMethodTests: AsyncNetworkManagerTestCase {
///
///     // Test method for the PATCH HTTP method with a specific endpoint.
...
final class PatchHTTPMethodTests: AsyncNetworkManagerTestCase {
    
    /// Test method for handling the successful scenario of updating a user's profile.
    ///
    /// This private test method demonstrates the process of updating a user's profile on the server. It makes an HTTP PATCH request with the provided endpoint details, where the endpoint path is set to "/api/users/2" to update the profile of the user with ID 2.
    ///
    ...
    ///
    /// The test method performs the HTTP PATCH request and awaits the response. The server is expected to respond with the updated user profile, and the test method validates the response by checking the updated name and job.
    ///
